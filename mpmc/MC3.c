 ...
 * Program to implement interrupts and ISR on PIC18f4550
 */
...
void main(void){
    TRISBbits.TRISB0 = 1;   //RB) as input
    TRISBbits.TRISB1 = 0;   //RB1 as output
    PORTBbits.RB1 = 1;      //RB1 as high so that LED turns on
    INTCONbits.GIE = 1;     //Enable Global Interrupt   
    INTCONbits.PEIE = 1;    //Enable peripheral interrupt
    INTCONbits.INT0E = 1;   //Enable RB0 interrupt
    
//    while(1);
    while(1){               //Infinite while loop waiting for interrupt
        INTCONbits.INT0F = 1;
        delay1s();
        INTCONbits.INT0F = 0;
        delay1s();
...
void __interrupt() ISR(void){
    ...
    //Check RB0 GPIO interrupt Pin
    if(INTCONbits.INT0F == 1){
        INTCONbits.INT0F = 0;   // set pin value to 0
        //If LED is ON turn it off
        if(PORTBbits.RB1 == 1){
            PORTBbits.RB1 = 0;
        }
        //Else turn on the LED if OFF
        else{
            PORTBbits.RB1 = 1;
        }
    }
